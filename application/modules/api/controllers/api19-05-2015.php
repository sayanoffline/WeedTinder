...
class api extends MY_Controller {
    ...
    public function __construct() {
		...
		$this->doc_path=realpath(APPPATH . '../assets/uploads/profile_images/');
	...
	$result = array();
		...
		foreach ($arr->profile_images as $key => $value) {
            ...
            /* Added by Chiranjib on 18-5-15 */
            
            // Get image string posted from Android App
	    $base = value;
	    // Get file name posted from Android App
	    $filename = strtotime(date('d-m-Y h:i:m')).$key.'.png';
	    
	    $profileImages[$key] = $filename;
	    // Decode Image
	    $binary=base64_decode($base);
	    header('Content-Type: bitmap; charset=utf-8');
	    // Images will be saved under 'www/imgupload/uplodedimages' folder
	    $file = fopen($this->doc_path.$filename, 'wb');
	    // Create File
	    fwrite($file, $binary);
	    fclose($file);
    ...
    public function update_gpsinfo() { 
		...
		$finalarr=$this->api_model->update_gpsinfo($data,$uid);
		if($finalarr=='1'){
            $result=array("message"=>'Success');
        }else{
            $result=array("message"=>'Error');
